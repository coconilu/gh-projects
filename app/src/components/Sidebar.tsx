...
export function LinkBadge({ link }: { link: LinkedWorkItem | null }) {
  ...
  return link.type === "issue" ? (
    <span className="badge b-issue" onClick={onClick}>● #{link.number}</span>
  ) : (
    <span className="badge b-pr" onClick={onClick}>⇄ !{link.number}</span>
  );
}
...
function CoBlock({ p, c, depth }: { p: Project; c: CheckoutInfo; depth: number }) {
  const { sel, setSel, setView, ci, openDialog, closeDialog, refreshProjects, toast } =
    useStore();
...
export default function Sidebar() {
  ...
  const addLocalFolder = () => {
    openDialog({
      kind: "prompt",
      title: "添加本地文件夹",
      message: "输入一个已存在的 git 仓库路径（任意 git 仓库；带 GitHub 远程时可管理 Issues/PR/Actions）",
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="group-by">
        ...
        <select
          value={groupBy}
          onChange={(e) => setGroupBy(e.target.value as "repo" | "status" | "ci")}
        >
          ...
          projects.map((p) => {
            ...
            return (
              <div className="proj" key={p.id}>
                  ...
                  <span className="acts">
                    {p.providerIdentity && (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          openUrl(`https://github.com/${p.providerIdentity!.owner}/${p.providerIdentity!.repo}`);
                        }}
                      >
                        ↗
                      </button>
                ...
                {open && p.checkouts.map((c) => <CoBlock key={c.id} p={p} c={c} depth={1} />)}
              </div>
            );
          })}
  ...
  for (const p of projects)
    for (const c of p.checkouts) {
      ...
      if (groupBy === "status") {
        ...
        const s = ci[c.id];
        k = s === "ok" ? "CI 通过" : s === "run" ? "CI 进行中" : s === "fail" ? "CI 失败" : "无 CI";
      }
      (buckets[k] ||= []).push({ p, c });
    }
  return (
