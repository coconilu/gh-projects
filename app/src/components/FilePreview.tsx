...
export default function FilePreview({ fkey, co }: { fkey: string; co: string }) {
  ...
  useEffect(() => {
    setPv(null);
    if (!abs) return;
    api.readFilePreview(abs).then(setPv).catch((e) => toast(`读取文件失败: ${e}`));
  }, [fkey, co]);
  ...
  return (
    <div className="content">
      ...
      <div className="preview-head">
        📄 {fname} <span style={{ color: "var(--faint)" }}>· 双击文件树可用编辑器打开</span>
        <span style={{ flex: 1 }} />
        <button className="btn sm" onClick={() => abs && api.openInEditor(abs).catch((e) => toast(`${e}`))}>
          在编辑器打开
        </button>
      </div>
      ...
      {pv && !pv.isBinary && isMd && (
        <div className="card md" dangerouslySetInnerHTML={{ __html: marked.parse(pv.text) as string }} />
      )}
      {pv && !pv.isBinary && !isMd && (
        <>
