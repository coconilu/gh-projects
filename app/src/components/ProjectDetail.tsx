...
import { abText, LinkBadge } from "./Sidebar";
...
function Overview({ p }: { p: Project }) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: 仅在项目切换（p.id 变化）时重新加载概览数据
	useEffect(() => {
		setReadme(null);
		setIssues(null);
		setPrs(null);
	...
	return (
		<>
			...
			{readme && (
				<div
					className="card md"
					// biome-ignore lint/security/noDangerouslySetInnerHtml: 本地 README 预览，内容由 marked 生成
...
function WorktreesBranches({ p }: { p: Project }) {
	...
	const load = () =>
		api
			...
			.then(setBranches)
			.catch((e) => toast(`${e}`));
	// biome-ignore lint/correctness/useExhaustiveDependencies: 仅在项目切换（p.id 变化）时重新加载分支列表
	useEffect(() => {
		setBranches(null);
		load();
	}, [p.id]);
	...
	const newWorktree = () => {
		openDialog({
			...
			title: "新建 worktree",
			message:
				"输入新分支名；将从默认分支创建，目录落在兄弟目录 <repo>.worktrees/<slug>/",
		...
		<>
			...
			<div className="two-col">
				<div>
					{p.checkouts.map((c) => (
						<div className="co-card" key={c.id}>
							<h4>
								...
								{c.isPrimary && (
									<span className="badge b-primary">primary</span>
								)}
					...
					<button className="btn" onClick={newWorktree}>
						＋ 新建 worktree
					</button>
				</div>
				<div>
...
function Issues({ p }: { p: Project }) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: 仅在项目切换（p.id 变化）时重新加载 issues
	useEffect(() => {
		setIssues(null);
		if (!gh) return;
		api
	...
	const linked = (n: number) =>
		p.checkouts.find(
			(c) =>
				c.linkedWorkItem?.type === "issue" && c.linkedWorkItem.number === n,
		);
						...
						{issues?.map((i) => {
							...
							return (
								<tr key={i.number}>
									...
									<td>
										<span
											className={i.state === "open" ? "st-open" : "st-closed"}
										>
											●
										</span>{" "}
										<a
											// biome-ignore lint/a11y/useValidAnchor: 点击经 openUrl 调系统浏览器，无 href
									...
									<td>
										{co ? (
											<span className="badge b-issue">⎇ {co.branch}</span>
										) : (
											"—"
										)}
									</td>
...
function Prs({ p }: { p: Project }) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: 仅在项目切换（p.id 变化）时重新加载 PR 列表
	useEffect(() => {
		setPrs(null);
		setChecks({});
		if (!gh) return;
		api
			...
			.then(async (list) => {
				...
				for (const pr of list.slice(0, 20)) {
					try {
						const r = await api.latestRunForBranch(
							gh.owner,
							gh.repo,
							pr.headRef,
						);
	...
	const linked = (n: number) =>
		p.checkouts.find(
			(c) => c.linkedWorkItem?.type === "pr" && c.linkedWorkItem.number === n,
		);
							...
							return (
								<tr key={pr.number}>
									...
									<td>
										...
										{pr.draft && (
											<span className="priv" style={{ marginLeft: 6 }}>
												draft
											</span>
										)}
									</td>
									...
									<td>
										{co ? (
											<span className="badge b-pr">⎇ {co.branch}</span>
										) : (
											"—"
										)}
									</td>
...
function Actions({ p }: { p: Project }) {
	...
	const loadRuns = (idx: number, list?: WorkflowInfo[]) => {
		...
		setRuns(null);
		api
			.listRuns(gh.owner, gh.repo, w.id)
			.then(setRuns)
			.catch((e) => toast(`${e}`));
	};

	// biome-ignore lint/correctness/useExhaustiveDependencies: 仅在项目切换（p.id 变化）时重新加载 workflows
	useEffect(() => {
		setWfs(null);
		setCur(0);
		if (!gh) return;
				...
				<div className="card" style={{ padding: "4px 0" }}>
					...
					{runs?.map((r) => (
						<div className="run" key={r.id}>
							...
							<button className="btn sm" onClick={() => openUrl(r.url)}>
								日志
							</button>
